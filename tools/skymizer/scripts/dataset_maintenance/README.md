...
```bash
uv venv "$SKYMIZER_WORK/dataset-maintenance-venv" --python 3.12.3
export MAINTENANCE_PYTHON="$SKYMIZER_WORK/dataset-maintenance-venv/bin/python"
uv pip install --python "$MAINTENANCE_PYTHON" -r tools/skymizer/scripts/dataset_maintenance/requirements.txt
"$MAINTENANCE_PYTHON" tools/skymizer/scripts/dataset_maintenance/scan.py \
...
"$MAINTENANCE_PYTHON" -m unittest discover \
  -s tools/skymizer/scripts/dataset_maintenance -p test_scan.py -v
```
...
sys.path.insert(0, 'tools/skymizer/scripts/dataset_maintenance')
...
args = SimpleNamespace(dataset='elichen-skymizer/vlm-prepared-dataset', revision='main',
...
"$MAINTENANCE_PYTHON" tools/skymizer/scripts/dataset_maintenance/replace_subsamples.py prepare \
...
```bash
"$MAINTENANCE_PYTHON" tools/skymizer/scripts/dataset_maintenance/replace_subsamples.py publish \
...
```bash
"$MAINTENANCE_PYTHON" tools/skymizer/scripts/dataset_maintenance/replace_subsamples.py verify \
...
```bash
"$MAINTENANCE_PYTHON" -m unittest discover \
  -s tools/skymizer/scripts/dataset_maintenance -p 'test_*.py' -v
```
