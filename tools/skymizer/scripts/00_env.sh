#!/usr/bin/env bash
# Shared wrapper settings. Model and dataset paths are relative to tools/skymizer.
# SKYMIZER_WORK selects build/venv/output defaults; relative work paths use the caller cwd.

skymizer_clean_cuda_path() {
...
BUILD_DIR=${BUILD_DIR:-${SKYMIZER_WORK:+$SKYMIZER_WORK/build}}
