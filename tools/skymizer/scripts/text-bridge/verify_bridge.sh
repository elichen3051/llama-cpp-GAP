...
#   CUDA_VISIBLE_DEVICES= python tools/skymizer/cli/verify_perplexity_bridge.py --prepared PREPARED \
...
( cd -- "$SKYMIZER_REPO" && run_logged "$LOG" env CUDA_VISIBLE_DEVICES= nice -n 10 \
    "$PYTHON" "$SKYMIZER_REPO/tools/skymizer/cli/verify_perplexity_bridge.py" \
