...
1. **Native runtime**: `llama-perplexity`, `llama-tokenize`, `llama-llm-kld` and their shared libraries.
   Either `RUNTIME_DIR` with `bin/` + `lib/` (the release package ships `runtime/`; `runtime.json` records
   its source commit, CUDA, GPU and the SHA256 of every file), or `TEXT_BIN`/`TEXT_LIB` pointing at a fresh
   `build/bin` made from the fork below with `cmake -DGGML_CUDA=ON -DCMAKE_BUILD_TYPE=Release` and targets
   `llama-tokenize llama-perplexity llama-llm-kld`.
2. **Fork checkout** (`SKYMIZER_REPO`): <https://github.com/elichen3051/llama-cpp-GAP>, branch
   `text-bridge-production-scripts`. Its `tools/skymizer/cli/{prepare_perplexity_corpus,collect_llm_kld,
   ...
   now committed in the fork). Auto-detected when these scripts run from `tools/skymizer/scripts/text-bridge`
   inside that checkout. Needed by stages 1, 4 and 5 only.
3. **Python** (`PYTHON`): the fork's locked environment, `uv sync --project tools/skymizer --python 3.12.3 --locked`
...
```bash
export RUNTIME_DIR=/path/to/runtime            # bin/ + lib/  (or TEXT_BIN=TEXT_LIB=/path/to/build/bin)
export SKYMIZER_REPO=/path/to/llama-cpp-GAP    # not needed when running from inside the fork
export PYTHON=/path/to/venv/bin/python
cd scripts                                     # or tools/skymizer/scripts/text-bridge in the fork
...
```bash
python tools/skymizer/cli/collect_llm_kld.py \
...
CUDA_VISIBLE_DEVICES= python tools/skymizer/cli/verify_perplexity_bridge.py \
...
The maintained copy of this directory is `tools/skymizer/scripts/text-bridge/` in the fork; the copy in the
