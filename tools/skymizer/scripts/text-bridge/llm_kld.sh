...
#   python tools/skymizer/cli/collect_llm_kld.py --ref-model REF --cand-model CAND \
...
( cd -- "$SKYMIZER_REPO" && run_logged "$LOG" "$PYTHON" "$SKYMIZER_REPO/tools/skymizer/cli/collect_llm_kld.py" \
...
( cd -- "$SKYMIZER_REPO/tools/skymizer" && CUDA_VISIBLE_DEVICES= "$PYTHON" - "$OUT" "$WAIVER" <<'PY'
