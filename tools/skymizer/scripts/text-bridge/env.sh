...
#   SKYMIZER_REPO  llama.cpp fork checkout providing tools/skymizer (auto-detected when this directory is
#                  tools/skymizer/scripts/text-bridge of that checkout). Needed by prepare_corpus.sh,
#                  llm_kld.sh and verify_bridge.sh only.
#   PYTHON         interpreter with tools/skymizer's locked dependencies (datasets, numpy, pyarrow, huggingface-hub)
...
# Two homes: <release>/scripts next to <release>/runtime, or <fork>/tools/skymizer/scripts/text-bridge.
_TB_REPO=$(cd -- "$_TB_SCRIPTS/../../../.." 2>/dev/null && pwd || true)
[[ -n "$_TB_REPO" && -f "$_TB_REPO/tools/skymizer/cli/collect_llm_kld.py" ]] || _TB_REPO=
export SKYMIZER_REPO=${SKYMIZER_REPO:-$_TB_REPO}
