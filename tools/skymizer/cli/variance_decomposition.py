...
Basic (kld, all stored positions):
    ...
    python3 cli/variance_decomposition.py \\
...
Reading it:
  ...
  * token-noise share > ~50% at the full cap -> extending --num-eval-tokens
    helps (check the collection-cap saturation line: extending adds tokens
    only to items that hit the collection cap);
  * share < ~30% -> only more items / a higher-signal dataset help.
