...
Review the scorer's prefix/window validation before its decode loop. Then inspect [core/skymizer-vlmk-kernel.h](../core/skymizer-vlmk-kernel.h), which owns the full-vocabulary metric computation and record layout. Keep its optimized path and naive oracle independent. The metric I/O definitions in [lib/kld_metrics_io.py](../lib/kld_metrics_io.py) must agree with that native layout.
...
`scripts/setup.sh` resolves cache/temp path overrides relative to the caller and places default build, Python, compiler, CUDA and HF caches under `SKYMIZER_WORK`. Runtime cache locations are separate from the numerical protocol; retain the actual executable/backend identity in each study.
