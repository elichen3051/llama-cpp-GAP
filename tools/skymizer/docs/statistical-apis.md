...
The power planner reuses the production paired-test interval. Its batched simulation retains vectorized item means and standard errors with SciPy t quantiles. The variance diagnostic now inverts the noncentral-t distribution for its SNR interval and searches integer sample sizes using exact two-sided power under an IID normal model. Local root brackets avoid unresolved remote tails; convergence warnings and failed tail residuals make results unavailable. Independent normal/chi-square quadrature validates the endpoints and required-N crossings. This conditional observed-effect diagnostic is not prospective power. The subsampling tool reuses the Wilson lower endpoint and labels nominal, unadjusted per-metric verdict agreement; its intervals and evaluated-grid crossings are pointwise.
...
Percentile and BCa now call SciPy bootstrap, including BCa midrank tie handling and reuse of an existing bootstrap distribution for CI inversion. Bootstrap-t remains explicit with NumPy crossed quantiles and refuses any unusable nonconstant pivot. Finite-resample tail support is checked at actual reported endpoints. Its censored p upper bound describes the fixed empirical CI family, not Monte Carlo uncertainty or population coverage. Optional unavailable cells keep their place in Holm families; malformed or nonfinite inputs still fail. Independent tied-sample symmetry, scalar inversion and adversarial rare-outlier tests validate the corrected contracts. The full-engine fixture uses 5000 draws because 1000 left only seven draws outside one BCa endpoint. Its item data are unchanged.

## Declared campaigns and selection

`stats.multiplicity.adjust_pvalues` delegates Holm/BH/BY to statsmodels with explicit method names. Original papers and SciPy/MATLAB/Julia correspondences are recorded in its docstring. The campaign loader verifies complete collection artifacts and reference/candidate fingerprints, then expands all planned quantization pairs before global adjustment. Intercept-only statsmodels OLS with CRV1 covariance keeps equal item weights and uses cluster count minus one for inference. Pointwise and Bonferroni simultaneous intervals are distinct outputs.

The deterministic subset selector uses a declared training-only maximin SNR objective. Source quotas, complete observed provenance and a frozen selection receipt constrain validation. This project-specific search is not supplied by a generic p-value package and does not establish generalization. See [campaign design and original papers](campaign-analysis.md) for schema, assumptions, fixed-N limits and independent verification.
