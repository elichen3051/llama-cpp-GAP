...
def load_cap_panel(args, caps):
    try:
        with paired_io.comparison_locks((args.candidate_a, args.candidate_b)):
...
def _load_cap_panel_locked(args, caps):
    ...
    _check_dirs(args)
    try:
        meta_a, meta_b, warnings, common_budget_skips = paired_io.validate_collection_pair(args.candidate_a, args.candidate_b)
    except (AlignmentError, ValueError) as error:
        sys.exit(str(error))
    matched, drops = paired_io.aligned_metric_items(args.candidate_a, args.candidate_b, allow_interaction=args.allow_interaction)
    if not matched:
    ...
    for key in matched:
        try:
            ma, ha, mb, hb = paired_io.load_item_pair(key, args.candidate_a, args.candidate_b)
        ...
        for cap in caps:
            try:
                score_a, score_b, _tok_a, _tok_b, keep, finite, drift, _versions = \
                    paired_io.score_records(key, ma, ha, mb, hb, cap)
