...
def main():
    ...
    parser.add_argument("--image-min-tokens", type=int, default=-1)
    parser.add_argument("--image-max-tokens", type=int, default=-1)
    ...
    if args.lane == "vlm":
        common += ["--ref-mmproj", args.mmproj, "--cand-mmproj", args.mmproj,
                   "--image-min-tokens", args.image_min_tokens,
    ...
    for name, other in (("paired", "b"), ("self-paired", "a-repeat")):
        ...
        if not args.verify_only:
            run([sys.executable, SKYMIZER / "cli/saved_metrics_paired_compare.py",
