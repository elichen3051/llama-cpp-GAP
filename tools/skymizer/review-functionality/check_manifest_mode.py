...
Validates the two correctness properties that the hermetic unit tests in
...
manifest-mode invocation) to confirm the model-reload saving. This script does
not exercise the opt-in multi-row wave path (`--n-seq-max > 1`), which is
expected to perturb logits by floating-point non-associativity.

Usage:
  python3 review-functionality/check_manifest_mode.py \
...
each must contain img_*.png, formatted_chat.txt, tokens.bin, meta.json (as
...
def main():
    ...
    for name in args.rows:
        ...
        imgs = sorted(str(pp) for pp in d.glob("img_*.png"))
        samples.append({
            "name": name,
            "images": imgs,
