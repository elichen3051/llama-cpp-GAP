...
SCHEMA = "skymizer-reference-freeze-v1"
...
def _selection(profiles_path, model, repo, revision, subset):
    ...
    _require(repo == f"elichen-skymizer/{model}-collect-400", "reference repo does not match the model and tail400 cohort")
...
def _validate_audits(out, profiles, model, repo, subset, mode):
    ...
    _require(plan.get("schema") == "skymizer-collect400-materialized-v1", "reference must use the materialized tail400 schema")
    _require(plan["destination"] == repo and plan["parent_repo"] == f"elichen-skymizer/{model}-collect-500"
             and plan["pilot_repo"] == f"elichen-skymizer/{model}-pilot"
