...
source "$SCRIPT_DIR/scripts/00_env.sh" --setup-only
...
log "uv sync --project $SCRIPT_DIR --python $PYTHON_VERSION --frozen --group dev"
uv sync --project "$SCRIPT_DIR" --python "$PYTHON_VERSION" --frozen --group dev
...
if [[ "${SKIP_TESTS:-0}" != 1 ]]; then
    ...
    (cd "$SCRIPT_DIR" && SKYMIZER_TEST_BIN="$BUILD_DIR/bin" uv run --project "$SCRIPT_DIR" --python "$VENV_DIR/bin/python" --no-sync \
