...
export TEXT_WORK="$COMPANY_WORK/qwen35-wikitext"

tools/gap/scripts/04_collect_text_bridge.sh
```
...
## Collections renamed after the fact

The published text-bridge collections were anonymized after collection: protocol labels such as the
`schema` string were renamed, while `protocol_sha256`, the window ids and metric file names derived from
it, `corpus_windows_sha256` and `dataset_content_hash` keep the values computed over the original labels.
`lib/text_corpus.py` recomputes those digests when loading a corpus map or a prepared dataset and rejects
a mismatch. Set `LEGACY_CORPUS_DIGEST=1` to accept such stored digests as opaque identifiers; the loader
then still requires consistent window order and offsets, ids that carry the stored digest prefix, matching
...
```bash
tools/gap/scripts/05_compare.sh text \
