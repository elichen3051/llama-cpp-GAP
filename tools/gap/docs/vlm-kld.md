...
```bash
export OUT_ROOT="$COMPANY_WORK/qwen35-pilot-vlm"
...
tools/gap/scripts/03_collect_vlm_kld.sh
tools/gap/scripts/05_compare.sh vlm
```
...
"$COMPANY_PYTHON" tools/gap/cli/collect_model_kld.py \
  --profiles "$COHORT_PROFILE" --study "$COMPANY_WORK/profile-study" --size 100 \
  ...
  --llama-vlm-kld "$COMPANY_WORK/build/bin/llama-vlm-kld" --dry-run
```
