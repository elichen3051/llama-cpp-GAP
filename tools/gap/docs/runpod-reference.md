...
```bash
export COHORT_PROFILE="$PWD/tools/gap/profiles/small-pilot100.json"
...
export GPU=0
export REFERENCE_OUT="$COMPANY_WORK/reference/qwen35-pilot-mmmu-vision-ins"

"$COMPANY_PYTHON" tools/gap/cli/restore_reference_models.py \
...
```bash
tools/gap/scripts/01_generate_reference.sh --dry-run
tools/gap/scripts/01_generate_reference.sh
```
...
"$COMPANY_PYTHON" tools/gap/cli/run_reference_campaign.py \
  ...
  --out "$COMPANY_WORK/reference-campaign" --no-upload
```
