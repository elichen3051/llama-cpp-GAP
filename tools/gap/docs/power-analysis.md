...
Run examples from the repository root after setting `COMPANY_PYTHON`, `A`, `B` and `REPORTS` as shown in the [README](../README.md). The numerical values below illustrate syntax; they are not a recommended effect size for every model.
...
```bash
"$COMPANY_PYTHON" tools/gap/stats/cli/variance_decomposition.py \
  --candidate-a "$A" --candidate-b "$B" \
...
```bash
"$COMPANY_PYTHON" tools/gap/stats/cli/random_subsample_power.py \
  --candidate-a "$A" --candidate-b "$B" \
