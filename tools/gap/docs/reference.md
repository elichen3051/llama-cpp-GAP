...
```bash
"$COMPANY_PYTHON" tools/gap/cli/generate_reference.py \
  --llama-reference "$COMPANY_WORK/build/bin/llama-reference" \
  --dataset /path/to/prepared-dataset --out "$COMPANY_WORK/reference-example" \
  --no-enable-thinking -- \
...
`"$COMPANY_WORK/build/bin/llama-reference" --help` for llama.cpp flags. The tool uses a context
...
```bash
"$COMPANY_WORK/build/bin/llama-reference" -m /models/model-bf16.gguf \
  --mmproj /models/mmproj.gguf -ngl all -c 8192 -b 2048 -ub 512 \
  -t 8 -tb 8 -fa on --fit off -ctk f16 -ctv f16 --describe > "$COMPANY_WORK/props.json"
```
...
The schema version is `company-reference-v2`; its implementation and validation
...
```bash
.venv/bin/python tools/gap/cli/collect_kld.py \
...
```bash
.venv/bin/python tools/gap/cli/collect_llm_kld.py \
...
"$COMPANY_PYTHON" tools/gap/cli/run_reference_campaign.py \
  ...
  --out "$COMPANY_WORK/reference-pilot" --size 100 --gpus 0 --hardware pro6000 \
  --llama-reference "$COMPANY_WORK/build/bin/llama-reference"
```
...
`upload_reference.py` validates cohort reconciliation, identities, source revision, templates, image policy, mode, caps and sampling before staging parquet. Publication uses `user-company/<checkpoint>-pilot` or `<checkpoint>-collect-500`; each source config gains `-ins` or `-think`, with split `train`. Only eligible rows enter the dataset; complete audit files record failures and exclusions.
