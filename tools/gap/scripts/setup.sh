...
# COMPANY_WORK sets build/venv/tmp/cache defaults; explicit path overrides take priority.
...
set -euo pipefail
if [[ "${1:-}" == --help ]]; then
    echo 'Set COMPANY_WORK to a writable work volume. Overrides: BUILD_DIR, VENV_DIR, TMPDIR, UV_CACHE_DIR, CCACHE_DIR, CCACHE_TEMPDIR, CUDA_CACHE_PATH, HF_HOME, XDG_CACHE_HOME, JOBS, PYTHON_VERSION.'
    echo 'Controls: FORCE_CPU=1, SKIP_APT=1, SKIP_BUILD=1, SKIP_TESTS=1. See docs/workflows.md.'
    exit 0
fi
: "${COMPANY_WORK:?set COMPANY_WORK to a writable work volume}"

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)"
source "$SCRIPT_DIR/00_env.sh" --setup-only
COMPANY_DIR="$(cd -- "$SCRIPT_DIR/.." &>/dev/null && pwd)"
REPO_ROOT="$(cd -- "$COMPANY_DIR/../.." &>/dev/null && pwd)"
BUILD_DIR="${BUILD_DIR:-${COMPANY_WORK:-$REPO_ROOT}/build}"
VENV_DIR="${VENV_DIR:-${UV_PROJECT_ENVIRONMENT:-${COMPANY_WORK:+$COMPANY_WORK/venv}}}"
VENV_DIR="${VENV_DIR:-$REPO_ROOT/.venv}"
TMPDIR="${TMPDIR:-${COMPANY_WORK:-$BUILD_DIR}/tmp}"
UV_CACHE_DIR="${UV_CACHE_DIR:-${COMPANY_WORK:-$BUILD_DIR}/uv-cache}"
CCACHE_DIR="${CCACHE_DIR:-$COMPANY_WORK/ccache}"
CCACHE_TEMPDIR="${CCACHE_TEMPDIR:-$TMPDIR/ccache}"
CUDA_CACHE_PATH="${CUDA_CACHE_PATH:-$COMPANY_WORK/cuda-cache}"
HF_HOME="${HF_HOME:-$COMPANY_WORK/hf}"
XDG_CACHE_HOME="${XDG_CACHE_HOME:-$COMPANY_WORK/xdg-cache}"
...
log "uv sync --project $COMPANY_DIR --python $PYTHON_VERSION --locked --group dev"
uv sync --project "$COMPANY_DIR" --python "$PYTHON_VERSION" --locked --group dev
...
if [[ "${SKIP_BUILD:-0}" != 1 ]]; then
    ...
    company_check_scorer "$BUILD_DIR/bin/llama-vlm-kld" llama-vlm-kld || die "llama-vlm-kld self-test failed"
    company_check_scorer "$BUILD_DIR/bin/llama-llm-kld" llama-llm-kld || die "llama-llm-kld self-test failed"
fi
...
if [[ "${SKIP_TESTS:-0}" != 1 ]]; then
    ...
    (cd "$COMPANY_DIR" && COMPANY_TEST_BIN="$BUILD_DIR/bin" uv run --project "$COMPANY_DIR" --python "$VENV_DIR/bin/python" --no-sync \
        python -m pytest -q -p no:cacheprovider -m "not external_model" --basetemp "$TMPDIR/company-pytest" tests/)
fi
