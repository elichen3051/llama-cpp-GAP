...
manifest = {"repo": "user-company/" + a.model + "-pilot", "subset": subset, "rows": completion["rows"], "split": "train",
...
def test_campaign_snapshot_is_atomic_and_verifies_full_source_provenance(tmp_path, monkeypatch):
    ...
    import cli.run_reference_campaign as campaign
    root = tmp_path / "repo"
    company = root / "tools/gap"
    for name in ("core", "cli", "lib", "stats", "scripts", "profiles"):
        (company / name).mkdir(parents=True)
        (company / name / "sample.py").write_text("pass\n")
    profiles = company / "profiles/reference_model_profiles.json"
    profiles.write_text("{}")
    ...
    monkeypatch.setattr(campaign, "COMPANY", company)
...
def test_campaign_snapshot_includes_statistics_dependencies(tmp_path, monkeypatch):
    ...
    profile = campaign.COMPANY / "profiles/small-pilot100.json"
    ...
    assert "company/stats/engine.py" in hashes
