...
of `user-company/GAP-mmmu-pro-standard-10 :: qwen3.5-4b-ins-gen-2048`;
tests/data/gap-ocrbench-v1-kimi-vl-a3b-ins/ holds three rows of
`user-company/kimi-vl-a3b-ins-full-ref-text-rec-dec ::
...
def test_reference_driver_filters_and_reconciles_out_of_order_results(tmp_path, monkeypatch):
    ...
    metadata.update(model_path=str(model), mmproj_path="", schema_version="company-reference-v2")
    ...
    assert (args.out / "scripts/lib/reference_study.py").read_bytes() == (gen.COMPANY / "lib/reference_study.py").read_bytes()
...
def upload_run(tmp_path):
    ...
    run = tmp_path / 'run'
    run.mkdir()
    profiles = {'dataset': {'repo': 'user-company/vlm-prepared-dataset', 'revision': 'pinned'},
...
def test_upload_cohort_names_and_image_parquet_roundtrip(upload_run, size, mode):
    ...
    assert manifest['repo'] == 'user-company/test-model-' + ('pilot' if size == 100 else 'collect-500')
...
def test_kld_launcher_uses_matching_runtime_without_generation_or_analysis(tmp_path, model, ubatch, mode, cap):
    ...
    assert flag("--subset") == subset and flag("--dataset") == f"user-company/{model}-pilot"
