...
CUDA_VISIBLE_DEVICES='' COMPANY_TEST_BIN="$COMPANY_WORK/build/bin" \
  ...
  -q -p no:cacheprovider -m 'not external_model' --basetemp "$COMPANY_WORK/pytest-validation"
```
...
```bash
"$COMPANY_PYTHON" tools/gap/verify_and_validation_scripts/smoke_kld.py \
  --bin-dir "$COMPANY_WORK/build/bin" --lane vlm \
  ...
  --out "$COMPANY_WORK/native-vlm-smoke"
```
