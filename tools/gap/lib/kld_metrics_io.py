...
#     v3  56-byte records: + three float32 after `ear` holding the RENORMALIZED
#         top-K EAR (named `ear_K_normalized` here, which is what they were;
#         a one-build interim layout)
#     v4  68-byte records: + float32 `ear_20`, `ear_10`, `ear_5` (sum of
#         min(p_ref, p_cand) over the reference's top-K slots with full-vocab
...
# kld_record struct in company-vlmk-kernel.h. v1 = 40 bytes; v2 adds float32
...
assert KLD_RECORD_DT_V5.itemsize == 76, "v5 record layout drifted from company-vlmk-kernel.h"
...
assert KLD_RECORD_DT_V4.itemsize == 68, "v4 record layout drifted from company-vlmk-kernel.h"
