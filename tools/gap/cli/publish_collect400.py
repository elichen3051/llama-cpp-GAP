...
def read_json(repo, revision, path, token):
    ...
    local = hf_hub_download(repo, path, repo_type="dataset", revision=revision,
                            token=token, cache_dir="/tmp/company-collect400-cache")
...
def stage(parent_repo, directory, token, revision=None):
    ...
    if not re.fullmatch(r"user-company/[A-Za-z0-9_.-]+-collect-500", parent_repo):
        raise ValueError("parent must be an user-company native collect-500 repo")
    ...
    for config in parent.card_data.to_dict()["configs"]:
        ...
        if (source["path"] != "user-company/vlm-prepared-dataset"
    ...
    plan = {"schema": "company-collect400-view-v1", "destination": destination,
...
def validate_stage(directory, plan):
    ...
    if (plan.get("schema") != "company-collect400-view-v1"
            or not re.fullmatch(r"user-company/[A-Za-z0-9_.-]+-collect-500", parent)
...
def check_manifest(manifest, repo, config):
    ...
    if (manifest.get("repo") != repo or manifest.get("subset") != config
            ...
            or source.get("path") != "user-company/vlm-prepared-dataset" or source.get("split") != "train"
