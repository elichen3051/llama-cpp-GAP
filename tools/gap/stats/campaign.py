...
def validate_plan(plan):
    ...
    if plan.get("schema_version") != "company-campaign-plan-v1":
        raise ValueError("schema_version must be company-campaign-plan-v1")
...
def analyze_campaign(plan, loaded):
    ...
    return {"schema_version": "company-campaign-result-v1", "plan_sha256": content_hash(plan), "plan": plan,
...
def validate_selection_receipt(plan, loaded):
    ...
    if receipt is None:
        return
    if receipt.get("schema_version") != "company-selection-v1":
...
def select_benchmark(plan, loaded, rule):
    ...
    receipt = {"schema_version": "company-selection-v1", "rule": rule, "training_plan_sha256": content_hash(plan),
