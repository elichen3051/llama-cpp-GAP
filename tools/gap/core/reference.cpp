...
struct reference_context {
    ...
    explicit reference_context(common_params p, bool stop_repetition) : params(std::move(p)), loaded(common_init_from_params(params)), repetition_stop(stop_repetition) {
        ...
        if (params.speculative.types == std::vector<common_speculative_type>{COMMON_SPECULATIVE_TYPE_DRAFT_MTP}) {
            ...
            if (params.speculative.has_dft()) {
                ...
                company_identity::check(company_identity::vocabulary(llama_model_get_vocab(loaded->model())),
                                        company_identity::vocabulary(llama_model_get_vocab(head.get())), true);
            }
        ...
        metadata = {
            {"schema_version", "company-reference-v2"}, {"generation_engine", "llama.cpp"},
            ...
            {"vocabulary", company_identity::vocabulary(vocab)},
    ...
    void emit(reference_row & row, llama_token token, double logprob) {
        ...
        if (repetition_stop && row.logprobs.size() % company_repetition::interval == 0) {
            record_repetition(row, company_repetition::tail(row.tokens.data() + row.n_prefill, row.logprobs.size()), "online");
    ...
    void record_repetition(reference_row & row, company_repetition::match match, const char * stage) {
        if (match) {
    ...
    json finish_row(reference_row & row) {
        if (repetition_stop && row.repetition.is_null()) {
            record_repetition(row, company_repetition::full(row.tokens.data() + row.n_prefill, row.logprobs.size()), "final");
        }
        row.result.update({
...
int main(int argc, char ** argv) {
    const int identity_command = company_identity::command(argc, argv);
    ...
    try {
        ...
        context.metadata["repetition_detector"] = {{"enabled", args.repetition_stop}, {"version", "exact-token-repeat-v1"},
            ...
            {"min_repeated_tokens", company_repetition::min_span}, {"min_repeats", company_repetition::min_repeats},
            {"online_window", company_repetition::window}, {"check_interval", company_repetition::interval},
