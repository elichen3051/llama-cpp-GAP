...
// Write per-answer-token metrics; metric computation and the VLMK layout are in company-vlmk-kernel.h.
//
// CLI:
...
int main(int argc, char ** argv) {
    ...
    try {
        std::vector<company_identity::json> expected;
        for (const auto & entry : manifest_entries) {
            if (entry.ok && !entry.reference_vocabulary.is_null()) {
                expected.push_back(entry.reference_vocabulary);
            }
        }
        company_identity::check_manifest(expected, args.ref_model_path, args.cand_model_path,
